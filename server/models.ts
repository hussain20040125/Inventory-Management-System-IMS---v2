...
const InventorySchema = new Schema({
  ...
  liveStock: { type: Number, default: 0 }, // Legacy/Compatibility (Map to availableQty + allocatedQty)
...
InventorySchema.pre('save', function(next: any) {
  // Sync logic: availableQty = liveStock - allocatedQty
...
export const Vendor = Supplier; // Legacy alias for safety during migration
...
const GRNSchema = new Schema({
  ...
  poId: String,
  project: String,
  destinationProject: String, // Added to match Inward
  supplier: String,
  date: String,
  challan: String,
  mrNo: String,
  gatePassNo: String, // Added to match Inward
  ...
  personName: String, // Added to match Inward
  personPhotoUrl: String, // Added to match Inward
  personPhotos: [String], // Added to match Inward
}, { timestamps: true });
...
// InwardSchema.index({ "items.sku": 1 });
...
const AuditLogSchema = new Schema({
  ...
  resource: { type: String, required: true }, // e.g., 'Inventory', 'User'
  resourceId: String,
...
const MaterialRequirementItemSchema = new Schema({
  ...
  qty: { type: Number, required: true }, // requested_qty
  unit: String,
...
const MaterialRequirementSchema = new Schema({
  id: { type: String, required: true, unique: true },
  mrNumber: String, // Added field
  engineerId: String, // Added field
...
QuotationSchema.pre('save', async function() {
  if (!this.token) {
    this.token = `QT-TOKEN-${(this as any).id || Math.random().toString(36).substr(2, 9)}-${Date.now()}`;
  }
});
